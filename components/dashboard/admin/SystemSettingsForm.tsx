  ...
  return (
    <div className="flex flex-col gap-8 w-full">
      <div className="bg-white border border-[#D3D3D3] border-[1.5px] rounded-[12px] p-6 shadow-sm">
          ...
          <table className="w-full text-left border-collapse min-w-[600px]">
            <thead>
              <tr className="bg-[#FAFBFC] border-b border-[#E8ECF0]">
                ...
                <th className="p-4 text-[#A0AEC0] text-[12px] font-normal uppercase">Description</th>
                <th className="p-4 text-[#A0AEC0] text-[12px] font-normal uppercase text-center w-24">Actions</th>
              </tr>
            </thead>
            <tbody>
              {settings.length === 0 ? (
                ...
                settings.map((setting) => (
                  <tr key={setting.key} className="border-b border-[#F4F6F8] hover:bg-gray-50 transition-colors">
                    <td className="p-4 font-bold text-[#0A1413] text-[14px] font-lato">{setting.key}</td>
                    <td className="p-4 text-[#6B7280] text-[14px] font-lato">{setting.value}</td>
                    <td className="p-4 text-[#6B7280] text-[12px] font-lato">{setting.description || '-'}</td>
        ...
        <form onSubmit={handleAdd} className="bg-[#F9FAFB] p-5 rounded-lg border border-[#E8ECF0]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[#0A1413] text-[12px] font-nunito">Description (Optional)</label>
              <input
                type="text"
                value={newDesc}
